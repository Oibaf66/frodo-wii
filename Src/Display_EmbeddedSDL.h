...
static u32 screenLock = 0;
lwpq_t ThreadOneQ;
lwp_t GUIthread;
#define THREADSTACK 1024*128	
...
bool start_GUI_thread()
{
	if (isGuiAvailable) // TODO && !hostScreen.isGUIopen())
	{
		...
		LWP_CreateThread (&GUIthread, open_gui, NULL, threadonestack, THREADSTACK, 80);
	}
...
void kill_GUI_thread()
{
	...
	//	SDL_KillThread(GUIthread);
		LWP_ThreadBroadcast(GUIthread);
		LWP_JoinThread(GUIthread, NULL);
		LWP_CloseQueue(GUIthread);
		GUIthread = NULL;
	}
}
...
void screenlock()
{
	...
	//}
	while(LWP_MutexLock(screenLock));
}
...
int init_graphics(void)
{
	...
	LWP_MutexInit (&screenLock, false);
...
uint8 C64::poll_joystick(int port)
{
	...
	if (joy[port]) {
		...
		if (SDL_JoystickGetButton(joy[port], 0)) // A button
		{
			j &= 0xef;							// Button

		}
		if (SDL_JoystickGetButton(joy[port], 6)) //Left Trigger
		{
			kbd_buf_feed("\rLOAD\"*\",8,1\rRUN\r");
		}
		
		if (SDL_JoystickGetButton(joy[port], 5)) //right trigger
		{
			     int F5=21;
			     kbd_buf_feed((char*)F5);

		}		
		if (SDL_JoystickGetButton(joy[port], 7)) //start button
		{
			//load_prg(this, (uint8*)brucelee_bin,size_brucelee_bin);
			//kbd_buf_feed("\rRUN\r"); 
			     int F7=23;
			     kbd_buf_feed((char*)F7);
		}
	}
